...
###########함수 라이브러리###########
from common.utils import hyundai_load, Annual_total_load, select_year
#####################################
...
def render_hyundai_page():
    ...
    # 2. 연도 선택 버튼
    with col_filter: 
        available_years = select_year(df_hyundai)
    ...
    if not df_hyundai.empty and not df_total.empty:
        ...
        if not market_share_df.empty:
            ...
            if selected_year == 2025:
                st.caption("※ 2025년 데이터는 연간추정치입니다.")
            ...
            if not selected_year_data.empty:
                ...
                if not prev_year_data.empty:
                    ...
                    # 증감율 계산 (2025년은 2025년 데이터를 2로 곱해서 계산)
                    if selected_year == 2025:
                        # 2025년 데이터를 2로 곱해서 연간 추정치로 계산
                        adjusted_hyundai_sales = hyundai_sales * 2
                        adjusted_total_new_count = total_new_count * 2
                        adjusted_market_share = (adjusted_hyundai_sales / adjusted_total_new_count * 100) if adjusted_total_new_count > 0 else 0
                        
                        hyundai_sales_change = ((adjusted_hyundai_sales - prev_hyundai_sales) / prev_hyundai_sales * 100) if prev_hyundai_sales > 0 else 0
                        total_new_count_change = ((adjusted_total_new_count - prev_total_new_count) / prev_total_new_count * 100) if prev_total_new_count > 0 else 0
                        market_share_change = adjusted_market_share - prev_market_share
                    else:
                        hyundai_sales_change = ((hyundai_sales - prev_hyundai_sales) / prev_hyundai_sales * 100) if prev_hyundai_sales > 0 else 0
                        total_new_count_change = ((total_new_count - prev_total_new_count) / prev_total_new_count * 100) if prev_total_new_count > 0 else 0
                        market_share_change = market_share - prev_market_share
                    # 요약문 생성
                    summary_text = f"""
                    **{selected_year}년 현대자동차 시장 점유율 요약"""
                    ...
                    summary_text += f"""
                    - 점유율: {adjusted_market_share if selected_year == 2025 else market_share:.1f}% (전년 대비 {market_share_change:+.1f}%p)
                    - 판매량: {int(adjusted_hyundai_sales if selected_year == 2025 else hyundai_sales):,}대 (전년 대비 {hyundai_sales_change:+.1f}%)
                    - 시장 규모: {int(adjusted_total_new_count if selected_year == 2025 else total_new_count):,}대 (전년 대비 {total_new_count_change:+.1f}%)
                    ...
                    if market_share_change > 0:
                        summary_text += f"- 전년 대비 점유율 {market_share_change:.1f}%p 상승으로 시장 경쟁력 강화\n"
                    else:
                        summary_text += f"- 전년 대비 점유율 {abs(market_share_change):.1f}%p 하락으로 시장 점유율 개선 필요\n"
                    
                    if hyundai_sales_change > 0:
                        summary_text += f"- 판매량 {hyundai_sales_change:.1f}% 증가로 성장세 지속\n"
                    else:
                        summary_text += f"- 판매량 {abs(hyundai_sales_change):.1f}% 감소로 판매 전략 재검토 필요\n"
                    
                    # 시장 상황 요약
                    if total_new_count_change > 0:
                        summary_text += f"- 전체 시장 {total_new_count_change:.1f}% 성장으로 시장 확대\n"
                    else:
                        summary_text += f"- 전체 시장 {abs(total_new_count_change):.1f}% 축소로 시장 환경 악화\n"
                    ...
                    # 시장 분석 요약
                    if market_share_change > 0:
                        if hyundai_sales_change > total_new_count_change:
                            summary_text += f"- 현대자동차가 시장 성장률({total_new_count_change:+.1f}%)을 상회하는 성장률({hyundai_sales_change:+.1f}%)을 기록하여 시장 점유율 상승\n"
                        else:
                            summary_text += f"- 시장 전체가 {total_new_count_change:+.1f}% 성장한 가운데, 현대자동차도 {hyundai_sales_change:+.1f}% 성장하여 점유율 상승\n"
                    else:
                        if hyundai_sales_change < total_new_count_change:
                            summary_text += f"- 시장 전체가 {total_new_count_change:+.1f}% 성장한 반면, 현대자동차는 {hyundai_sales_change:+.1f}% 성장하여 점유율 하락\n"
                        else:
                            summary_text += f"- 시장 전체가 {total_new_count_change:+.1f}% 감소한 가운데, 현대자동차는 {hyundai_sales_change:+.1f}% 감소하여 점유율 하락\n"
                    
                     # 경쟁력 평가 요약
                    current_market_share = adjusted_market_share if selected_year == 2025 else market_share
                    if current_market_share >= 40:
                        summary_text += f"- 시장 점유율 {current_market_share:.1f}%로 강력한 시장 지배력 보유\n"
                    elif current_market_share >= 30:
                        summary_text += f"- 시장 점유율 {current_market_share:.1f}%로 시장에서 강세 유지\n"
                    elif current_market_share >= 20:
                        summary_text += f"- 시장 점유율 {current_market_share:.1f}%로 안정적인 시장 위치 차지\n"
                    else:
                        summary_text += f"- 시장 점유율 {current_market_share:.1f}%로 점유율 확대 필요\n"
                    ...
                    # 전년도 데이터가 없는 경우
                    st.markdown("---")
                    st.subheader("📊 현대자동차 점유율 분석")
                    
                    current_analysis = f"""
                    **{selected_year}년 현대자동차 시장 점유율 분석"""
                    
                    if selected_year == 2025:
                        current_analysis += f""" (연간추정치)"""
                    
                    current_analysis += f"""
                    
                    📈 **현재 상황**
                    - 현대자동차 판매실적: {int(hyundai_sales):,}대
                    - 전체 신차 등록현황: {int(total_new_count):,}대
                    - 현대자동차 점유율: {market_share:.1f}%
                    - 기타 제조사 점유율: {other_share:.1f}%
                    ...
                    # 경쟁력 평가
                    if market_share >= 40:
                        current_analysis += "- **시장 지배력**: 현대자동차는 시장에서 강력한 지배력을 보이고 있습니다.\n"
                    elif market_share >= 30:
                        current_analysis += "- **강세**: 현대자동차는 시장에서 강세를 유지하고 있습니다.\n"
                    elif market_share >= 20:
                        current_analysis += "- **안정적**: 현대자동차는 시장에서 안정적인 위치를 차지하고 있습니다.\n"
                    else:
                        current_analysis += "- **도전적**: 현대자동차는 시장 점유율 확대가 필요한 상황입니다.\n"
                    ...
                    **{selected_year}년 현대자동차 시장 점유율 요약"""
                    ...
                    if selected_year == 2025:
                        summary_current += f""" (연간추정치)"""
                    
                    summary_current += f"""
                    
                    🎯 **핵심 지표**
                    - 점유율: {market_share:.1f}%
                    - 판매량: {int(hyundai_sales):,}대
                    - 시장 규모: {int(total_new_count):,}대
                    
                    📈 **주요 특징**
                    - {selected_year}년 기준 현대자동차의 시장 점유율은 {market_share:.1f}%로 시장에서 {'강력한 지배력을 보이고 있습니다' if market_share >= 40 else '강세를 유지하고 있습니다' if market_share >= 30 else '안정적인 위치를 차지하고 있습니다' if market_share >= 20 else '점유율 확대가 필요한 상황입니다'}
                    ...
                    # 경쟁력 평가 요약
                    if market_share >= 40:
                        summary_current += f"- 시장 점유율 {market_share:.1f}%로 강력한 시장 지배력 보유\n"
                    elif market_share >= 30:
                        summary_current += f"- 시장 점유율 {market_share:.1f}%로 시장에서 강세 유지\n"
                    elif market_share >= 20:
                        summary_current += f"- 시장 점유율 {market_share:.1f}%로 안정적인 시장 위치 차지\n"
                    else:
                        summary_current += f"- 시장 점유율 {market_share:.1f}%로 점유율 확대 필요\n"
                    
                    st.markdown(summary_current)
